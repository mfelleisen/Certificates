...
(define 0certificate%
  (class object% (init-field a)
    ...
    #; {Natural Natural -> Void}
    (define/public (swap i j) (void))

    #; {[Vector Real] -> Boolean}
    (define/public (validator b)
      (eprintf "this use of `sort` does not allow the generation of a certificate")
      #true)))
...
(define contract-for-self-certifying-sort
  (->i ([a [vectorof real?]])
       ([s contract-for-swap])
       #:param (a s) record-swap-events
       (new (if (unsupplied-arg? s) 0certificate% certificate%) [a a])
       (b [vectorof real?])
...
(define/contract (bubble-sort a (swap swap))
